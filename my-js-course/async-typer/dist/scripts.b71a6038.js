...
console.log(getRandomBetween(0, 100)); // & async for of loop
...
function draw(el) {
  ...
  var _el$dataset = el.dataset,
      typeMin = _el$dataset.typeMin,
      typeMax = _el$dataset.typeMax;

  function drawLetter() {
    return _drawLetter.apply(this, arguments);
  } // when this function draw() runs, kick off drawLetter


  function _drawLetter() {
    _drawLetter = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
      var amountOfTimeToWait;
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              el.textContent = text.slice(0, index);
              index += 1; // exit condition

              amountOfTimeToWait = getRandomBetween(typeMin, typeMax);
              _context.next = 5;
              return wait(amountOfTimeToWait);

            case 5:
              if (index <= text.length) {
                drawLetter(); // wait for some time
